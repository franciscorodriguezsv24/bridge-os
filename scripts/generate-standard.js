...
// Design OS exports tokens as CSS files:
...
//   'primary': '#6366F1',           (Tailwind config)
function parseCSSTokens(css) {
  ...
  // Tailwind color map fallback: 'key': '#value'
  if (rows.length === 0) {
...
function buildStandard({ tokens, components, exportPath }) {
  ...
  lines.push(`2. **Use exported components** — If a component exists in \`design-export/sections/\``);
  lines.push(`   or \`design-export/shell/\`, import and use it. Do not re-implement it.`);
  lines.push(`3. **Check user flows first** — Before implementing any screen, read`);
  lines.push(`   \`design-export/product-overview.md\` to understand requirements and flows.`);
  lines.push(`4. **Shell is the layout** — The app shell from \`design-export/shell/\``);
  ...
  lines.push(``);
  lines.push(`The following components are exported and ready to use.`);
